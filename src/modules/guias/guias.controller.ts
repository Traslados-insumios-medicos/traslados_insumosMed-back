  ...
  try {
    ...
    if (req.user?.rol === "CLIENTE") {
      ...
      if (!ids.includes(guia.clienteId)) {
        res
          .status(403)
          .json({ message: "No tiene permisos para ver esta guía" });
        return;
      }
    }
    res.json(guia);
...
export const getMisEnvios = async (
  ...
  try {
    ...
    const limit = Math.max(
      1,
      Math.min(100, parseInt(String(req.query.limit), 10) || 10),
    );
    const search =
      typeof req.query.search === "string" ? req.query.search : undefined;
    const rawVista =
      typeof req.query.vista === "string" ? req.query.vista : "todos";
    const vista: VistaMisEnvios = [
      "activos",
      "historial",
      "todos",
      "incidencias",
